...
import { Building2, Radio, AlertTriangle, MapPin, Eye, EyeOff, Search, X, Layers, ZoomIn, ZoomOut, Crosshair } from 'lucide-react';
...
export default function FullMapPage() {
    ...
    // Map state
    const [center, setCenter] = useState<[number, number]>([13.7563, 100.5018]); // Thailand center
        ...
        const fetchData = async () => {
            try {
                ...
                if (bureauRes.data) {
                    ...
                    // Extract provinces from bureaus
    ...
    const provinceCenter = useMemo(() => {
        if (selectedProvince) {
            ...
            if (provinceStations.length > 0) {
                return { lat: provinceStations[0].latitude, lng: provinceStations[0].longitude };
            }
        }
        return null;
    ...
    useEffect(() => {
        if (provinceCenter) {
            setCenter([provinceCenter.lat, provinceCenter.lng]);
            setZoom(10);
        }
    }, [provinceCenter]);
    ...
    return (
        <div className="h-[calc(100vh-64px)] w-full relative bg-gray-950">
            ...
            {/* Selected Station Info (Bottom Left) */}
            {selectedStation && (
            ...
            <div className="absolute bottom-4 right-4 z-[500] flex gap-3">
                ...
                <div className="bg-gray-900/95 border border-gray-700 rounded-lg px-3 py-2 text-center">
                    ...
                    <p className="text-[10px] text-gray-500">กำลังลาดตระเวน</p>
                </div>
                <div className="bg-gray-900/95 border border-gray-700 rounded-lg px-3 py-2 text-center">
            ...
            >
                ...
                {/* Risk Zones (only when zoomed in) */}
